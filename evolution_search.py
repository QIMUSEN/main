...
def parse_cmd_options(argv):
    ...
    parser.add_argument('--llm_provider', type=str, default='deepseek', choices=['deepseek', 'qwen'],
                        help='The LLM provider to use for architecture generation.')
    module_opt, _ = parser.parse_known_args(argv)
    return module_opt

def generate_by_llm(structure_str, args):
    ...
    if args.llm_provider == 'deepseek':
        ...
        model_name = os.getenv("QWEN_MODEL", "qwen-long") # e.g., qwen-long, qwen-turbo, etc.
        if not api_key:
            raise ValueError("QWEN_API_KEY not found in .env file or environment variables.")
    else:
    ...
    prompt = template.replace("{structure_str}", structure_str)

    response = client.chat.completions.create(
        model=model_name,
        messages=[
            {"role": "system", "content": "You are an expert in Neural Architecture Search."},
    ...
    # Print the provider and the exact model version returned by the API
    print(f"[INFO] LLM provider: '{args.llm_provider}', Model Version Used: '{response.model}'")

    raw_response = response.choices[0].message.content

    import json
    # Clean up the response string, removing potential markdown code blocks
    cleaned_response = raw_response.strip().replace('`', '')
    if cleaned_response.startswith('json'):
        cleaned_response = cleaned_response[4:].strip()

    try:
        # Try to parse the response as JSON and extract the 'arch' key
        data = json.loads(cleaned_response)
        new_structure_str = data['arch']
    except (json.JSONDecodeError, KeyError):
        # If it's not a valid JSON or doesn't have 'arch', use the cleaned response directly
        new_structure_str = cleaned_response

    return new_structure_str

def get_new_random_structure_str(AnyPlainNet, structure_str, num_classes, get_search_space_func,
                                 num_replaces=1, args=None):
    ...
    get_new_llm_structure_str = generate_by_llm(structure_str, args)

    with open('llm_output.log', 'w') as f:
        f.write(get_new_llm_structure_str)
...
def get_splitted_structure_str(AnyPlainNet, structure_str, num_classes):
    try:
        ...
        print(f"\n[DEBUG] PARSING FAILED for LLM output: {structure_str}\n")
...
def main(args, argv):
    ...
    # Check if the search space file provides a structure, otherwise use a default one
    if hasattr(select_search_space, 'plainnet_struct'):
        initial_structure_str = select_search_space.plainnet_struct
    else:
        # Define a default starting architecture if none is provided
        initial_structure_str = "SuperConvK3BNRELU(3,32,1,1)SuperResK1K3K1(32,128,2,32,4)SuperResK3K3(128,256,2,64,4)SuperResK1K5K1(256,512,2,128,4)SuperConvK1BNRELU(512,1024,1,1)"

    masternet = AnyPlainNet(num_classes=args.num_classes, opt=args, argv=argv, no_create=True, plainnet_struct=initial_structure_str)
    ...
    for loop_count in range(args.evolution_max_iter):
        ...
        if len(popu_structure_list) <= 10:
            random_structure_str = get_new_random_structure_str(
                ...
                get_search_space_func=select_search_space.gen_search_space, num_replaces=1, args=args)
        else:
            ...
            random_structure_str = get_new_random_structure_str(
                ...
                get_search_space_func=select_search_space.gen_search_space, num_replaces=2, args=args)
...
# validate
        ...
        logging.info(f"Iter {loop_count}/{args.evolution_max_iter} | Score: {the_nas_core:.4f} | Arch: {random_structure_str}")
